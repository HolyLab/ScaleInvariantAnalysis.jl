...
struct ShermanMorrisonMatrix{T<:Number, AT<:AbstractMatrix, UT<:AbstractVector{T}, VT<:AbstractVector{T}} <: AbstractMatrix{T}
    A::AT
    u::UT
    v::VT
    A⁻¹u::UT
    ...
    function ShermanMorrisonMatrix{T, AT, UT, VT}(A, u, v) where {T<:Number, AT<:AbstractMatrix, UT<:AbstractVector{T}, VT<:AbstractVector{T}}
...
ShermanMorrisonMatrix(A, u, v) = ShermanMorrisonMatrix{promote_type(eltype(A), eltype(u), eltype(v))}(A, u, v)
